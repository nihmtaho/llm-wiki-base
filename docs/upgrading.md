...
llm-wiki upgrade --to latest    # backup → sync → stamp VERSION
...
1. Backs up managed files (`.agents/skills/`, `AGENTS.md`, `_schema.md`,
   `.claude/CLAUDE.md`) into `.llm-wiki/backups/<timestamp>/` (keeps 5).
2. Re-runs the same installers `init` uses (overwrite + prune shipped-away
   skills via `.agents/skills/.llm-wiki-skills.json`; your own skills kept).
3. Stamps the tag into `.llm-wiki/VERSION`.

Project wikis additionally re-sync the `codebase` skill (`llm-wiki-research`,
...
- `latest` only ever matches **stable** tags; betas are tried via explicit
  checkout (see skill `llm-wiki-release`).
- Rollback = `upgrade --to <older-tag>` (core checked out there first) or copy
  back from `.llm-wiki/backups/<ts>/`.

## Migrating from pre-tag versions

```bash
pip install -e . && llm-wiki base install           # 1. sync code → ~/.llm-wiki-base/
cd <old-wiki> && llm-wiki reindex --full             # 2. build chunks_fts + fix FTS duplicate rows
cd <old-wiki> && llm-wiki init personal -c claude    # 3. refresh skills + MCP (answer yes)
llm-wiki doctor                                     # 4. see what's left
```
...
- **Merged + renamed skills**: 13 personal/project skills → 8 profile-aware
  `llm-wiki-*`. Re-init **deletes** old `wiki-project-*` and keeps your own
  skills. Project wikis additionally get `llm-wiki-research` at the **repo
  root**.
...
- `--no-register` (and env `LLM_WIKI_REGISTRY`) keeps tests off the real registry.
...
- `llm-wiki proposals …` is new for `.proposals/`; old proposals need explicit
  `apply --target <path>`.
...
llm-wiki reindex          # wrapper uses the global base
```
