    ...
    if link.is_symlink():
        ...
        if _is_ours(link, src, mode):
            ...
            print(f"  ! {client}: giữ nguyên {link} "
                  f"(không phải bản do llm-wiki cài) — skill này KHÔNG được link.")
            return link
...
def _is_ours(link: Path, src: Path, mode: str) -> bool:
    """Link/dir này do llm-wiki ghi ra? = nội dung khớp bản trong `.agents/skills/`."""
    try:
...
def _prune(skills_dir: Path, wanted: list[str],
    ...
    """Xoá skill do llm-wiki cài mà bản này không còn ship. Không đụng skill của user."""
