...
while IFS="$(printf '\t')" read -r name src; do
  ...
  if [ -L "$claude_link" ]; then
    ...
    [ "$abs_target" = "$src" ] || { rm_link "$claude_link"; ln -s "$src" "$claude_link"; log "relink $name -> $src"; }
  ...
  if [ -L "$oc_link" ]; then
    ...
    [ "$abs_target" = "$skill_md" ] || { rm_link "$oc_link"; ln -s "$skill_md" "$oc_link"; log "relink $name.md -> $skill_md"; }
  elif [ -e "$oc_link" ]; then
    log "skip $name.md (.opencode: real file exists)"
  else
    ln -s "$skill_md" "$oc_link"
