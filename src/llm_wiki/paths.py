...
# Repo root — derived from this file's location. For src-layout, parent of src/llm_wiki/paths.py
# is <root>/src/llm_wiki/ → go up 3 levels to reach <root>. For tools/paths.py shim,
# `__file__` is the tools/ copy and parent is <root>/tools/ → go up 1.
_this = Path(__file__).resolve()
if _this.parent.name == "llm_wiki" and _this.parent.parent.name == "src":
    _DEFAULT_ROOT = _this.parent.parent.parent  # .../src/llm_wiki/paths.py → .../
else:
