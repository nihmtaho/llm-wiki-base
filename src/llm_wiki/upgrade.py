...
- State mỗi wiki: 1 dòng tag trong `.llm-wiki/VERSION`.
...
SRC_DIRS = ("src/llm_wiki/skills", "src/llm_wiki/templates/agents")
...
def backup_wiki(wiki: Path, ts: str | None = None) -> Path | None:
    """Copy managed hiện có vào `.llm-wiki/backups/<ts>/`, tỉa giữ 5 bản."""
    targets = managed_targets(wiki)
    if not targets:
        return None
    ts = ts or datetime.now().strftime("%Y%m%d-%H%M%S")
    dest = wiki / ".llm-wiki" / "backups" / ts
    for rel in targets:
        src = wiki / rel
        out = dest / rel
...
def backup_root_skills(root: Path, ts: str) -> Path | None:
    """Backup `.agents/skills` của root vào `<root>/.llm-wiki/backups/<ts>/`."""
    if not (root / SKILLS_DIR).is_dir():
        return None
    ts_dir = root / ".llm-wiki" / "backups" / ts
    dest = ts_dir / SKILLS_DIR
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(root / SKILLS_DIR, dest, symlinks=True)
    for old in sorted((root / ".llm-wiki" / "backups").iterdir())[:-BACKUP_KEEP]:
        shutil.rmtree(old, ignore_errors=True)
    return ts_dir
...
def changed_sources(core: Path, old: str | None, new: str) -> list[str] | None:
    ...
    for line in proc.stdout.decode().splitlines():
        line = line.strip()
        if line.startswith("src/llm_wiki/skills/"):
            rest = line.split("src/llm_wiki/skills/", 1)[1].split("/", 2)
            if len(rest) == 3:  # <subset>/<skill>/<file...>
                mapped.append(str(Path(".agents/skills") / rest[1] / rest[2]))
        elif line.startswith("src/llm_wiki/templates/agents/"):
