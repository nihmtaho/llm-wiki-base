...
def cmd_init(cfg: dict) -> int:
    ...
    if src:
        ...
        text = (
            "# eval/golden.toml — bộ query vàng cho `llm-wiki-base eval`.\n"
            "# Chưa tìm thấy template trong global base — chạy `llm-wiki-base base install`.\n\n"
    ...
    with open(dst, "w", encoding="utf-8") as f:
        f.write(text)
    print(f"đã tạo {dst} — sửa lại thành query THẬT của bạn rồi chạy `llm-wiki-base eval`")
    return 0
...
def main() -> int:
    ...
    if not queries:
        ...
        for e in errors:
            print(f"  - {e}")
        print(f"  Sửa/tạo: {golden_file}  (hoặc chạy `llm-wiki-base eval --init`)")
    ...
    if chunks == 0 and base_settings["chunk_bm25"]:
        print("[warn] chunks_fts rỗng → kênh bm25_chunk đang tắt; "
              "chạy `llm-wiki-base reindex --full` rồi đo lại")
        ...
        for name, overrides in PROFILES:
            ...
            if overrides.get("vector") and not _vector_available():
                print(f"[{name}] SKIPPED — chưa có rag index "
                      "(`vector = true` + `llm-wiki-base reindex --full` rồi đo lại)")
                continue
            _with_vector(s)
...
def _verdict(runs: list, k: int) -> None:
    ...
    if vec is None:
        print("  → chưa đo được kênh vector: `vector = true` + `llm-wiki-base reindex --full`")
        return
    base = runs[0]["aggregate"]
    if vec["aggregate"]["r_at_k"] > base["r_at_k"] + 0.02:
        print("  → vector cải thiện recall: cân nhắc đặt `vector = true` trong .llm-wiki-base.toml")
    else:
