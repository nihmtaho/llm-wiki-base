...
tools/ chạy từ global venv (~/.llm-wiki-base/.venv) KHÔNG có package `llm_wiki_base`
→ fallback về bản inline dưới đây. Giữ đồng bộ phần read-only với
src/llm_wiki_base/config_file.py (write-only như tomli_w nằm ở package side).
"""
try:
    from llm_wiki_base.config_file import DEFAULTS, effective, get_config  # noqa: F401
except ImportError:
    ...
    # PHẢI KHỚP KEY-FOR-KEY với llm_wiki_base/config_file.py DEFAULTS (bản package).
    # `llm-wiki-base doctor` so 2 bản và báo lỗi nếu lệch — lệch ở đây nghĩa là CLI và
