...
def format_meta(meta: dict) -> str:
    ...
    return f"<!-- llm-wiki-base-proposal\n{lines}\n-->\n"
...
def cmd_list(_: argparse.Namespace) -> int:
    ...
    if not rows:
        ...
        print("Proposal được tạo bởi MCP tool `wiki_propose_edit` hoặc `llm-wiki-base proposals new`.")
        return 0
    ...
    print()
    print("Xem diff: `llm-wiki-base proposals show <name>`")
    print("Duyệt   : `llm-wiki-base proposals apply <name> --by <human-id>`")
    print("Bỏ      : `llm-wiki-base proposals discard <name>`")
    return 0
...
def cmd_apply(a: argparse.Namespace) -> int:
    ...
    if dest is None:
        ...
        print("  Xác nhận tường minh: llm-wiki-base proposals apply "
    ...
    print(f"  llm-wiki-base verify {rel} --by <human-id>")
    return 0
...
def cmd_new(a: argparse.Namespace) -> int:
    ...
    print(f"  xem: llm-wiki-base proposals show {prop.name}")
    return 0
...
def _resolve(name: str) -> Path | None:
    ...
    if len(matches) == 1:
        return matches[0]
    if not matches:
        print(f"[ERROR] không tìm thấy proposal '{name}'. Chạy `llm-wiki-base proposals list`.")
    else:
...
def _reindex(rel: str, dest: Path) -> None:
    ...
    try:
        ...
        print("  Chạy: llm-wiki-base reindex")
