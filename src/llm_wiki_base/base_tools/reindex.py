...
Chế độ:
    ...
    reindex --full     — rebuild toàn bộ từ đầu (bỏ qua hash). Bắt buộc sau khi
                         đổi embed_model/chunk_tokens/vector trong .llm-wiki-base.toml,
...
def main():
    ...
    if args.check:
        ...
        if not n_chunks and wiki_pages:
            print(
                ...
                "kênh bm25_chunk đang tắt; chạy `llm-wiki-base reindex --full`"
            )
        else:
    ...
    if drift and not args.full:
        print("[warn] config đổi so với lần reindex trước ("
              ...
              "content-hash, nên chạy `llm-wiki-base reindex --full` để rebuild "
