...
def _set_wiki_ctx(wiki_name: str) -> tuple[str, Path]:
    ...
    if entry is None:
        raise ValueError(
            ...
            f"Chạy `llm-wiki-base wiki list` để xem wikis đã đăng ký."
        )
...
def registry_list() -> str:
    ...
    wikis = _load_registry()
    if not wikis:
        return "(registry rỗng — chạy `llm-wiki-base init` để đăng ký wiki)"
    lines = []
    for w in wikis:
...
def wiki_search(query: str, top_k: int = 8, wiki: str = "") -> list:
    ...
    if wiki:
        ...
        if not wikis_to_search:
            return [_err("registry rỗng — chạy `llm-wiki-base init` để đăng ký wiki")]
...
def semantic_search(query: str, top_k: int = 6, wiki: str = "") -> list:
    """Semantic chunk-level search thô (cần `llm-wiki-base reindex` với vector=true).
...
def wiki_submit(
    ...
    entry = _wiki_entry(wiki)
    if entry is None:
        return _err(f"wiki '{wiki}' không có trong registry. Chạy `llm-wiki-base wiki list`")
