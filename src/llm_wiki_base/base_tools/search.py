...
log = logging.getLogger("llm-wiki-base.search")
...
# để `llm-wiki-base eval` in ra. Xoá sạch mỗi lần hybrid_search chạy.
CHANNEL_ERRORS: dict = {}
...
def sync_chunks(conn, page_id: int, path: str, content: str, chunk_tokens: int) -> int:
    ...
    # chunk mất hút với ingest đơn file (`llm-wiki-base ingest`).
    conn.commit()
    return n
...
def _warn_no_chunks():
    ...
    log.warning("chunks_fts rỗng — chạy `llm-wiki-base reindex --full` để bật kênh bm25_chunk")
