...
`llm-wiki-base setup doctor` lists exactly that boundary when you are unsure.
"""
import os
import shutil
import subprocess
...
# Hidden pre-redesign group (spec §7): no callback — bare `llm-wiki-base init`
...
init_app = typer.Typer(
    ...
    hidden=True,
    epilog="Examples:\n  llm-wiki-base init\n  llm-wiki-base setup personal --name notes --no-mcp",
)

setup_app = typer.Typer(
    help="Set up wikis, tools and health checks",
    epilog="Examples:\n  llm-wiki-base setup\n  llm-wiki-base setup personal --name notes --yes",
)
...
config_app = typer.Typer(help="Behavior config per-wiki (.llm-wiki-base.toml)")
...
def wiki_list_cmd() -> None:
    ...
    """
    from llm_wiki_base.registry import list_wikis
    wikis = list_wikis()
    if not wikis:
        if _ui.is_quiet():
            return
        console.print("[dim]Registry is empty. Run `llm-wiki-base setup` to create your first wiki.[/dim]")
        return
    _ui.wiki_table(wikis)
    if any(not w.get("id") for w in wikis):
        console.print("[dim]Empty ID → re-run `llm-wiki-base wiki add`.[/dim]")
    console.print()
...
def wiki_add_cmd(
    ...
    Examples:
        llm-wiki-base wiki add notes /path/to/notes
        llm-wiki-base wiki add docs /path/to/repo/project-wiki --type project
    """
    from llm_wiki_base.registry import add_wiki
    wiki_path = Path(path).resolve()
    if not wiki_path.exists():
        _ui.err_panel(f"path does not exist: {wiki_path}", "llm-wiki-base wiki list")
        raise typer.Exit(1)
...
def wiki_remove_cmd(
    ...
    Examples:
        llm-wiki-base wiki remove notes
        llm-wiki-base wiki remove notes --force
    """
    from llm_wiki_base.registry import find, remove_wiki
    if not find(name):
...
def base_install(
    ...
    Run once after `pip install llm-wiki-base`. Idempotent — re-run to sync the
...
def _run_base_tool(tool_name: str, args: list[str], wiki_root: Path,
    ...
    if not script.exists():
        _ui.err_panel(f"{script} does not exist.", "llm-wiki-base setup tools")
        raise typer.Exit(1)
    if not Path(py).exists():
        _ui.err_panel(f"missing base venv python: {py}", "llm-wiki-base setup tools --force")
        raise typer.Exit(1)
    ...
    try:
        ...
        _ui.err_panel(f"`llm-wiki-base {tool_name.replace('.py', '')}` "
                      f"failed (exit {e.returncode}).",
                      "llm-wiki-base wiki reindex --check, then llm-wiki-base setup doctor")
...
def ingest_cmd(
    ...
    Examples:
        llm-wiki-base wiki ingest raw/inbox/foo.md
        llm-wiki-base wiki ingest raw/inbox/foo.md --root /path/to/wiki
    """
    target = Path(path)
...
def reindex_cmd(
    ...
    Examples:
        llm-wiki-base wiki reindex
        llm-wiki-base wiki reindex --check
        llm-wiki-base wiki reindex --full
    """
    args = []
    if full:
        args.append("--full")
...
def lint_cmd(
    ...
    Examples:
        llm-wiki-base check lint
        llm-wiki-base check lint --fix
    """
    args = ["--root", str(root)]
    if fix:
        args.append("--fix")
...
def eval_cmd(
    ...
    Examples:
        llm-wiki-base check eval
        llm-wiki-base check eval --compare
        llm-wiki-base check eval --k 5 --verbose
    """
    args = []
    if k:
        args += ["--k", str(k)]
...
def watch_cmd(
    ...
    Examples:
        llm-wiki-base watch
        llm-wiki-base watch --root /path/to/wiki
    """
...
def serve_cmd(
    ...
    """Run the MCP server for AI tools — same as `llm-wiki-base serve --mcp`.
    ...
    points at this command: ["llm-wiki-base", "serve", "--mcp"].

    Examples:
        llm-wiki-base serve --mcp
    """
    if not mcp:
        _ui.err_panel("`llm-wiki-base serve` needs flag --mcp (stdio MCP server for AI tools).",
                      "llm-wiki-base serve --mcp")
        raise typer.Exit(2)
    from llm_wiki_base.installer import CENTRALIZED_SERVER_SCRIPT
    base = get_base_dir()
...
def translate_enable(
    ...
    """Enable auto-translation. Writes target langs to `.llm-wiki-base.toml` at the wiki root.

    After enabling, the ingest skill auto-calls the `llm-wiki-base-translate` skill to
    translate each new source into the target langs (using the running AI tool's LLM).

    Examples:
        llm-wiki-base translate enable --lang vi
        llm-wiki-base translate enable --lang vi --lang ja
    """
    from llm_wiki_base.config_file import set_translate
...
def translate_disable(
    ...
    Examples:
        llm-wiki-base translate disable
    """
    from llm_wiki_base.config_file import get_translate_config, set_translate
    _, langs = get_translate_config(root)
    set_translate(root, enabled=False, langs=langs)
    if langs:
        console.print(f"[green]✓[/green] disabled (langs kept: {langs}). Re-enable with `llm-wiki-base translate enable`.")
    else:
...
def translate_status(
    ...
    Examples:
        llm-wiki-base translate status
    """
...
def translate_check(
    ...
    Examples:
        llm-wiki-base translate check --lang vi
    """
...
# làm `.llm-wiki-base.toml` bị vô hiệu riêng trong MCP (env > TOML). Xem installer.py.
...
def config_show(
    ...
    """Print the effective config: builtin defaults + .llm-wiki-base.toml + env overrides.

    Each line is tagged with its source: [default] / [toml] / [env ...].

    Examples:
        llm-wiki-base config show
        llm-wiki-base config show --root /path/to/wiki
    """
    from llm_wiki_base.config_file import get_config, load
    cfg = get_config(root)
    raw = load(root)
    if not raw:
        console.print("[dim](no .llm-wiki-base.toml — using all defaults)[/dim]\n")
    _ui.config_table(_collect_config(cfg, raw))
    console.print()
    console.print("To change a value: edit [cyan].llm-wiki-base.toml[/cyan] at the wiki root (env vars override when set).")
...
def verify_cmd(
    ...
    Examples:
        llm-wiki-base check verify wiki/tech/concept/x.md --by nihmtaho
        llm-wiki-base check verify wiki/tech/concept/x.md --unverify
    """
    if not unverify and not by:
        _ui.err_panel("missing --by (reviewing human id). Use --unverify to drop verified.",
                      "llm-wiki-base check verify <path> --by <id>")
        raise typer.Exit(1)
    from llm_wiki_base import verify as verify_mod
    try:
        if unverify:
        ...
        _ui.err_panel(f"{e}", "llm-wiki-base check verify --help")
        raise typer.Exit(1)
...
def proposals_show(
    ...
    Examples:
        llm-wiki-base review show my-proposal
        llm-wiki-base review show my-proposal --context 5
    """
...
def proposals_apply(
    ...
    page stays unverified and you review later with `llm-wiki-base check verify`.

    Examples:
        llm-wiki-base review apply my-proposal
        llm-wiki-base review apply my-proposal --by nihmtaho
    """
    args = ["apply", name]
    if target:
        args += ["--target", target]
    ...
    if not applied:
        console.print("[yellow]![/yellow] cannot read the target path from output — review manually: "
                      f"`llm-wiki-base check verify <path> --by {by}`")
        return
    from llm_wiki_base import verify as verify_mod
    try:
        ...
        console.print(f"  Run manually: llm-wiki-base check verify <path> --by {by}")
...
def proposals_new(
    ...
    Examples:
        echo "new content" | llm-wiki-base review new --target wiki/tech/concept/x.md
        llm-wiki-base review new --target wiki/tech/concept/x.md --file /tmp/draft.md --by opencode
    """
...
def proposals_discard(
    ...
    Examples:
        llm-wiki-base review discard my-proposal --force
    """
    args = ["discard", name]
    if force:
        args.append("--force")
...
_SKILL_LAYER = [
    ("ingest into wiki pages", "llm-wiki-base-ingest",
     "`llm-wiki-base wiki ingest` only INDEXES 1 file into the DB; the skill WRITES the page."),
    ("answer questions + rerank", "llm-wiki-base-query",
     "retrieval is MCP/CLI, but candidate scoring and answer synthesis need the AI tool's LLM."),
    ("semantic review (contradictions, stale)", "llm-wiki-base-review",
     "`llm-wiki-base check lint` only checks deterministic structure."),
    ("consolidate / merge concepts", "llm-wiki-base-consolidate", "additive merge + distill-verify."),
    ("translate pages", "llm-wiki-base-translate",
     "`llm-wiki-base translate enable` only writes config; the skill creates translations."),
    ("cross-wiki research", "llm-wiki-base-research", "installed at the codebase root, not inside a wiki."),
]
...
def doctor_cmd(
    ...
    Examples:
        ...
        llm-wiki-base setup doctor --root /path/to/wiki
    """
    ...
    if base.exists():
        ...
        fail(f"base runtime not installed: {base} → run `llm-wiki-base setup tools`")
    py = get_base_python()
    ok(f"base python: {py}") if py.exists() else fail(f"missing base venv python: {py} → `llm-wiki-base setup tools --force`")
    on_path = shutil.which("llm-wiki-base")
    if on_path:
        ok(f"llm-wiki-base on PATH: {on_path}")
    else:
        warn("`llm-wiki-base` not on PATH (it lives in the install venv's bin/) → symlink it "
             "into /usr/local/bin or export PATH; see README “Making `llm-wiki-base` available globally”")
    ...
    if src_tools.is_dir() and (base / "tools").is_dir():
        ...
        if stale:
            warn(f"base tools/ DRIFTED from the package ({len(stale)} files) → run "
                 f"`llm-wiki-base setup tools`: {', '.join(stale[:6])}")
        else:
    ...
    if py311:
        ...
        checks.append(("tomli", "Python <3.11 needs it to read .llm-wiki-base.toml"))
    ...
    ok(f"{len(wikis)} wikis registered") if wikis else warn(
        "registry is empty → MCP sees no wikis; run `llm-wiki-base setup`")
    for w in wikis:
        if not Path(w.get("path", "")).exists():
            fail(f"'{w.get('name')}' points at a path that no longer exists: {w.get('path')} "
                 f"→ `llm-wiki-base wiki remove {w.get('name')}`")
        if not w.get("id"):
            warn(f"'{w.get('name')}' has no id (UUID) — re-add it with `llm-wiki-base wiki add`")

    _ui.section("Current wiki")
    cfg = root / ".llm-wiki-base.toml"
    if cfg.is_file():
        ok(f".llm-wiki-base.toml: {cfg}")
        from llm_wiki_base.config_file import load as load_cfg
        ...
        ok(f"\\[wiki].profile = {prof}") if prof else warn(
            ...
            "`llm-wiki-base setup personal|project` (it is idempotent)")
    else:
        warn(f"no .llm-wiki-base.toml at {root} (using defaults; is this the wiki root?)")
    ...
    if props:
        warn(f"{len(props)} proposals awaiting review → `llm-wiki-base review list`")
    else:
...
def upgrade_cmd(
    ...
    Examples:
        llm-wiki-base upgrade --dry-run
        llm-wiki-base upgrade --to latest
        llm-wiki-base upgrade --to v0.2.0 --wiki my-wiki
    """
    from llm_wiki_base import upgrade as _upgrade
    ...
    if result["unknown"]:
        _ui.err_panel(f"No wiki '{wiki}' in the registry.", "llm-wiki-base wiki list")
        raise typer.Exit(1)
    if dry_run:
        rows = []
