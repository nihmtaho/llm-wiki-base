...
Usage:
    llm-wiki-base init personal [--name <wiki-name>] [--client claude] [--no-mcp] [--no-skills]
...
def run(
    ...
    _ui.banner(f"Init personal wiki: {wiki_name}",
               f"at [cyan]{cwd}[/cyan]\n"
    ...
    # 4. Agent config files (AGENTS.md + _schema.md ở root, CLAUDE.md ở .claude/)
    ...
    # 5b. .llm-wiki-base.toml (behavior config — commit vào wiki repo)
    if template_exists("templates", "llm-wiki-base.toml") and not (cwd / ".llm-wiki-base.toml").exists():
        (cwd / ".llm-wiki-base.toml").write_text(
    ...
    if ensure_wiki_identity(cwd / ".llm-wiki-base.toml", profile="personal", lang=lang):
        console.print("  [green]✓[/green] .llm-wiki-base.toml: \\[wiki].profile = personal"
                      + (f", lang = {lang}" if lang else ""))

    # 5c. eval/golden.toml — bộ query vàng cho `llm-wiki-base eval` (commit vào wiki repo)
    ...
    rows = [
        ...
        f"Registry:     '{wiki_name}' in centralized MCP registry",
    ]
    for res in mcp_results:
        rows.append(f"MCP {res.client:<11} {res.describe()}")
    if skip_mcp:
        rows.append("MCP:          [dim]bỏ qua[/dim]")
    elif mcp_failed:
    ...
    console.print("  2. Ingest thật sự = chạy skill [bold]llm-wiki-base-ingest[/bold] trong AI tool "
                  "(CLI `llm-wiki-base ingest` chỉ index file đã có, không viết page)")
