"""Read/write `.llm-wiki-base.toml` ở wiki root.
...
sạch comment. Mọi write vào `.llm-wiki-base.toml` đã tồn tại đi qua `_upsert_toml_keys`
...
CONFIG_FILE = ".llm-wiki-base.toml"

# Defaults cho mọi section. File `.llm-wiki-base.toml` chỉ cần ghi đè key muốn đổi.
DEFAULTS: dict = {
    "wiki": {
    ...
    "models": {
        ...
        # AI tool đang mở (`llm-wiki-base doctor` nhắc rõ điều đó). Có section này để:
    ...
    "eval": {
        "k": 8,                            # cutoff mặc định của `llm-wiki-base eval`
...
def load(wiki_root: Path) -> dict:
    """Load `.llm-wiki-base.toml`. Trả dict rỗng nếu file không tồn tại."""
...
def get_config(wiki_root: Path) -> dict:
    """Effective config: DEFAULTS deep-merged với `.llm-wiki-base.toml` (nếu có)."""
...
def set_translate(wiki_root: Path, enabled: bool, langs: Iterable[str]) -> Path:
    ...
    Bản cũ parse-then-dump (`_write_toml`) → mọi comment trong `.llm-wiki-base.toml` bốc
