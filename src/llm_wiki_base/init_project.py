...
Usage:
    llm-wiki-base init project [--root <dir>] [--wiki-dir <name>] [--client claude] [--no-skills] [--no-mcp]
...
Project wiki chỉ chứa data + .env + .llm-wiki-base.toml + .agents/skills/.
"""
...
RESEARCH_START = "<!-- LLM_WIKI_BASE_RESEARCH_START -->"
RESEARCH_END = "<!-- LLM_WIKI_BASE_RESEARCH_END -->"
...
def run(
    ...
    if not base_dir.exists():
        _ui.err_panel(f"Global base chưa cài: {base_dir}", "llm-wiki-base setup tools")
        raise typer.Exit(1)
    ...
    # 4b. .llm-wiki-base.toml (behavior config — commit vào wiki repo)
    if template_exists("templates", "llm-wiki-base.toml") and not (wiki_dir / ".llm-wiki-base.toml").exists():
        (wiki_dir / ".llm-wiki-base.toml").write_text(
            read_template("templates", "llm-wiki-base.toml"), encoding="utf-8"
        )
        console.print("  [green]✓[/green] .llm-wiki-base.toml (wiki config — commit file này)")
    # Project wiki PHẢI là profile codebase (template mặc định là personal).
    if ensure_wiki_identity(wiki_dir / ".llm-wiki-base.toml", profile="codebase", lang=lang):
        console.print("  [green]✓[/green] .llm-wiki-base.toml: \\[wiki].profile = codebase"
                      + (f", lang = {lang}" if lang else ""))

    # 4c. eval/golden.toml — bộ query vàng cho `llm-wiki-base eval` (commit vào wiki repo)
    ...
    if skills_target != "skip":
        ...
        if not installed_wiki and not installed_root:
            console.print("  [yellow]![/yellow] không cài được skill nào — kiểm tra "
                          "package data (`llm-wiki-base base install` lại sau khi nâng cấp)")
    ...
    console.print(f"  2. Research xuyên wiki từ repo này: /llm-wiki-base-research <câu hỏi> "
                  f"(skill ở {root}/.agents/skills/)")
    console.print(f"  3. Hoặc MCP tool: wiki_search(query, wiki='{wiki_name}') — "
                  "để wiki=\"\" để search mọi wiki")
    console.print("  4. Đăng ký wiki khác (nếu có): [cyan]llm-wiki-base wiki add <name> <path> --type project[/cyan]")
