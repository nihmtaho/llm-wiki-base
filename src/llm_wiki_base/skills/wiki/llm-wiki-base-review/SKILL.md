...
Principle: **deterministic first, generative second.** `llm-wiki-base-lint` handles structure (runs FIRST); this skill handles what needs understanding.

## Cadence gate (cost control)

- Read the watermark `wiki/.review_state.json` (`last_run`). Full run only past `[review].interval_days` (`.llm-wiki-base.toml`); not due → deterministic skip (~0s), report "review not due".
...
1. Pick candidates per the scope above. Read pages + `wiki/pins.yml` + `status: open` gaps in `wiki/alerts/` (plus the code-path list handed over by `llm-wiki-base-lint` step 3, for the codebase profile).
2. **Semantic check** — every gap needs **EVIDENCE**: verbatim quotes + related Concept IDs.
   ...
   - **Trust gap** — canonical concept sitting `draft`/unverified too long → suggest human review: `llm-wiki-base verify <path> --by <id>`.
