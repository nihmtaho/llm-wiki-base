...
```bash
llm-wiki-base verify wiki/<domain>/concept/<slug>.md --by <human-id>   # set verified → human-reviewed
llm-wiki-base verify <path> --unverify                                 # drop verified (back to unverified)
```
...
- Threshold: below ~100k tokens BM25 suffices. `vector = true` **only when**
  `llm-wiki-base eval --compare` shows recall/MRR gains.
- Config: `[retrieval]` in `.llm-wiki-base.toml` (`mode`, `fusion`, `rrf_k`,
