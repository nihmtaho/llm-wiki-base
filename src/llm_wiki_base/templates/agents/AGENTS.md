...
- `eval/golden.toml` — golden retrieval queries (`llm-wiki-base eval`). **Data, COMMIT**. `eval/results.json` — measurement history, gitignored.
...
- Frontmatter: `title`, `domain`, `kind`, `tags`, `sources`, `updated`, `status` (+ legacy `confidence`); trust `generated`/`verified` (absent = unverified) + optional `stale_after`, `x_owner`, `x_supersedes`. **Never set `verified` yourself** (`llm-wiki-base verify … --by <id>`).
...
- **Per-wiki data** (`raw/`, `wiki/`, `rag/.rag_index/`, `.env`, `.llm-wiki-base.toml`). No code, no venv.
- Run the `llm-wiki-base <ingest|reindex|lint|watch>` wrappers (they call the global base). Never assume `tools/` lives inside the wiki.
...
| skill | scope | job |
|---|---|---|
| `llm-wiki-base-ingest` | wiki | raw → source/entity/concept pages + index/log + reindex |
| `llm-wiki-base-query` | wiki | answer from **this** wiki (retrieval → rerank → cite) |
| `llm-wiki-base-lint` | wiki | deterministic health-check + `--fix` for re-derivable content |
| `llm-wiki-base-reindex` | wiki | build/diagnose derived indexes (`--check`/`--full`) |
